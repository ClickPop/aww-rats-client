...
describe("Rat", () => {
  ...
  before(async () => {
    ...
    contract = await Rat.deploy("", "", weth.address, 0, 99, 1, 0, "AwwRat", "RAT").then(r => r.deployed());
  ...
  it("Should allow you to query all public methods/variables on the contract", async () => {
      ...
      const defaultMaxTokensPerWallet = await contract.defaultMaxTokensPerWallet();
      ...
      expect(defaultTokenUri).to.eq("https://awwrats.com/opensea-metadata.json");
      const contractURI = await contract.contractURI();
      expect(contractURI).to.eq("https://www.awwrats.com/default-erc721-token-metadata.json");
  ...
  it("Should allow you to transfer an nft from one wallet to another", async () => {
      await contract.transferFrom(user.address, user2.address, 0).then(t => t.wait());
