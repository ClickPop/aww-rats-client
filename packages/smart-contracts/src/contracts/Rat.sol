...
contract Rat is ERC721URIStorage, Ownable {
  ...
  mapping(address => uint256[]) private _burnedTokensByOwner;
  ...
  event TokenBurned(uint256 tokenId, address tokenOwner);
  event TokenTransferred(
    uint256 tokenId,
    address newOwner,
    address oldOwner,
    uint256[] newOwnerTokens,
    uint256[] oldOwnerTokens,
    address[] tokenOwners
  );
  ...
  constructor(
    ...
    uint256 baseId,
    uint32 _maxTokens,
    uint32 _defaultMaxTokensPerWallet,
    uint256 _cost,
    string memory name,
    string memory symbol
  ) ERC721(name, symbol) {
    _tokenIds = baseId != 0 ? baseId : 0;
    _contractURI = bytes(initContractURI).length != 0
      ? initContractURI
      : _contractURI;
    maxTokens = maxTokens != 100 ? _maxTokens : maxTokens;
    defaultTokenURI = bytes(_defaultTokenURI).length != 0
      ? _defaultTokenURI
      : defaultTokenURI;
    defaultMaxTokensPerWallet = _defaultMaxTokensPerWallet != 1
  ...
  function createToken() public payable {
    ...
    uint256 erc20Bal = erc20.balanceOf(msg.sender);
    ...
    require(
      balanceOf(msg.sender) < maxTokensPerWallet[msg.sender] ||
  ...
  function storeAsset(uint256 id, string memory uri) public onlyOwner {
    _setTokenURI(id, uri);
  }
  ...
  function burnToken(uint256 id) public {
    ...
    require(
      ownerOf(id) == msg.sender || isApprovedForAll(ownerOf(id), msg.sender),
      'You do not own this rat'
    );
  ...
  */
  function getBurnedTokensByOwner(address user)
    public
    view
  ...
  function setMaxTokensForWallets(address[] memory wallets, uint32 max)
    public
    onlyOwner
  {
  ...
  function setWalletBan(address[] memory wallets, bool banned)
    public
    onlyOwner
  {
    for (uint256 i = 0; i < wallets.length; i++) {
  ...
  function setDefaultMaxTokensPerWallet(uint32 _defaultMaxTokensPerWallet)
    public
    onlyOwner
  {
  ...
  */
  ...
  function removeTokenId(address addr, uint256 tokenId) internal {
    uint256 numTokensByOwner = _tokensByOwner[addr].length;
  ...
  function removeTokenHolder(address tokenOwner) internal {
    uint256 numHolders = tokenOwners.length;
